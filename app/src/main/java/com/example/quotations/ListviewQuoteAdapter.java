...
public class ListviewQuoteAdapter extends BaseAdapter {
    ...
    public ListviewQuoteAdapter(Activity a, List<Quotation> quotes) {
        activity = a;
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        if (QuotationsApplication.likes != null) {
            for (int i = 0; i < QuotationsApplication.likes.size(); i++) {
                // if this quote is in user's like list
                String ss = ((Quotation)QuotationsApplication.likes.get(i).getQuote()).getObjectId();
        ...
        if(QuotationsApplication.favorites != null) {
            for (int i = 0; i < QuotationsApplication.favorites.size(); i++) {
                // if this quote is in user's favorite list
                String ss = ((Quotation)QuotationsApplication.favorites.get(i).getQuote()).getObjectId();
                String sg = quotes.get(position).getObjectId();
                if (ss.equals(sg))
